...
export function printParserOutput(input: {
    ...
    input.tokens.forEach(element => {
        tokens.push({type: TokenType[element.type], line: element.line, start: element.start, length: element.length, lexeme: text.slice(element.start, element.start + element.length)})
    });
